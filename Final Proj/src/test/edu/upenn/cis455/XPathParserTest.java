...
public class XPathParserTest extends TestCase {
	...
	public void testValidXPath1() {
		Step step = validator.getStep("/bookstore[book[title]]");
		parser.setDocumentAndXPath(dom, step);
		assertTrue(parser.evaluate().size() > 0);
	}
	@Test
	public void testValidXPath2() {
		Step step = validator.getStep("/bookstore[book[title[@lang=\"en\"]]]");
		parser.setDocumentAndXPath(dom, step);
		assertTrue(parser.evaluate().size() > 0);
	}
	@Test
	public void testValidXPath3() {
		Step step = validator.getStep("/bookstore/book[@category=\"CHILDREN\"]/title[@lang=\"en\"]");
		parser.setDocumentAndXPath(dom, step);
		assertTrue(parser.evaluate().size() > 0);
	}
	@Test
	public void testValidXPath4() {
		Step step = validator.getStep("/bookstore/book[@category=\"CHILDREN\"]/title[text()=\"Harry Potter\"]");
		parser.setDocumentAndXPath(dom, step);
		assertTrue(parser.evaluate().size() > 0);
	}
	@Test
